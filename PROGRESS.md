...
| 2026-09-18 | Blocklists widened from 81k to 305k so the switch blocks **app** ads, not just browser ads | StevenBlack is web-first. With it alone, `ads.applovin.com`, `unityads`, `mopub` and friends resolved normally — the switch blocked banners on the web and quietly did nothing inside half the apps a user opens, which is not what "ad blocker" means to anyone. oisd big and HaGeZi Pro are the mobile-aware ones. Cost is 48 MB RSS, against 516 MB free. A short inline list carries in-app hosts the curated lists verifiably miss |
| 2026-09-18 | `imasdk.googleapis.com` is deliberately NOT blocked | It is Google's video-ad framework, and players commonly refuse to start the CONTENT when it fails to load — so blocking it costs the user the video, not the ad. The curated lists leave it alone for the same reason |
| 2026-09-18 | Found `clientGroupsBlock` commented out by hand in the node's `config.yml` | Ad blocking had been silently off: 81k domains loaded and applied to nobody, every metric green, `BLOCKED` counter flat at zero. Only symptom was ads getting through. `config.yml` is generated by the ops scripts and must not be hand-edited — `allowlist.txt` is the file for that, and the scripts never touch it |
